...
public class LinuxMediaProvider implements LocalMediaProvider {
    ...
    private static final String SCRIPT = "if command -v playerctl &>/dev/null; then P=$(playerctl -l 2>/dev/null|head -1); if [ -n \"$P\" ]; then echo \"TITLE=$(playerctl -p $P metadata title 2>/dev/null)\"; echo \"ARTIST=$(playerctl -p $P metadata artist 2>/dev/null)\"; echo \"ALBUM=$(playerctl -p $P metadata album 2>/dev/null)\"; echo \"STATUS=$(playerctl -p $P status 2>/dev/null)\"; echo \"SOURCE=$P\"; else echo NO_SESSION; fi; else echo NO_SESSION; fi";
    ...
    private Optional<NowPlayingTrack> parse(String out) {
        ...
        return Optional.of(new NowPlayingTrack(t, a, d.get("ALBUM"), st, d.getOrDefault("SOURCE", "Unknown")));
    }
